    ...
    <div class="container mx-auto px-6 max-w-7xl relative z-10">
      ...
      <div v-if="products.length > 0" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
        <NuxtLink
          ...
          class="group glass-panel flex flex-col h-full bg-slate-950/30 hover:bg-slate-950/50 border border-slate-800 hover:border-red-500/40 rounded-[2rem] overflow-hidden shadow-xl hover:shadow-red-950/10 hover:scale-[1.02] active:scale-98 transition-all duration-300"
          data-aos="fade-up"
          :data-aos-delay="idx * 50"
        >
          <!-- Product Image Wrapper -->
          <div class="relative aspect-square overflow-hidden bg-slate-950/50 border-b border-slate-900 flex items-center justify-center p-6">
            <img 
              v-if="product.image" 
              :src="product.image" 
          ...
          <div class="p-6 flex-grow flex flex-col justify-between space-y-4">
            <div class="space-y-2">
              <h3 class="text-lg font-black text-white leading-tight group-hover:text-red-500 transition-colors">
                {{ locale === 'nl' ? product.title_nl : product.title_en }}
              </h3>
              <p class="text-xs text-slate-400 line-clamp-2 font-medium">
                {{ locale === 'nl' ? product.description_nl : product.description_en }}
              </p>
            </div>

            <!-- Price and Action -->
            <div class="flex items-center justify-between pt-4 border-t border-slate-900/60 mt-auto">
              <div class="flex flex-col">
                <span class="text-[10px] uppercase font-bold text-slate-500 tracking-wider">{{ $t('products.price') }}</span>
                <span class="text-xl font-black text-white">€{{ formatPrice(product.price) }}</span>
              </div>
              <div class="w-10 h-10 bg-slate-900 border border-slate-800 rounded-xl flex items-center justify-center text-slate-300 group-hover:bg-red-600 group-hover:border-red-500 group-hover:text-white transition-all duration-300">
      ...
      <!-- No Products State -->
      <div v-else class="text-center py-20 bg-slate-950/20 rounded-[2.5rem] border border-slate-900/60 max-w-xl mx-auto space-y-4" data-aos="zoom-in">
        ...
        <p class="text-slate-400 text-sm font-bold">{{ $t('products.no_products') }}</p>
      </div>
...
<style scoped>
.glass-panel {
  @apply bg-slate-950/50 border border-slate-800/80 rounded-[2rem] backdrop-blur-xl shadow-2xl;
}
</style>
