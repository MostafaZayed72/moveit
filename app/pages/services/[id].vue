<template>
  ...
  <div v-else>
    ...
    <div v-if="dbService" class="pt-24 min-h-screen bg-slate-50 dark:bg-slate-950 transition-colors duration-300 font-sans">
      <!-- HERO SECTION WITH PREMIUM BACKGROUND -->
      <section class="relative h-[60vh] md:h-[70vh] flex items-center justify-center overflow-hidden">
        <div class="absolute inset-0 z-0">
          <img 
            :src="dbService.image || 'https://images.unsplash.com/photo-1497366216548-37526070297c?auto=format&fit=crop&q=80&w=1200'" 
        ...
        <div class="container mx-auto px-6 relative z-10 max-w-4xl text-center" data-aos="zoom-out">
          ...
          <h1 class="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black text-white mb-6 leading-tight tracking-tight drop-shadow-2xl">
      ...
      <section class="py-16 px-6 relative z-20">
        <div class="container mx-auto max-w-6xl space-y-24">
          <!-- 3-Column Service Quick Stats Grid (Decorative fallback stats for db services) -->
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16 -mt-24 relative z-30">
            <div class="premium-card p-8 flex items-center gap-6 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200/50 dark:border-white/5 shadow-xl rounded-3xl" data-aos="fade-up">
              <div class="w-14 h-14 rounded-2xl bg-red-500/10 flex items-center justify-center text-red-500 text-3xl font-semibold shadow-inner">🤝</div>
              <div>
                <div class="text-3xl font-black text-slate-900 dark:text-white">100%</div>
                <div class="text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">{{ isNl ? 'رعاية واحترام' : 'Care & Respect' }}</div>
              </div>
            </div>
            <div class="premium-card p-8 flex items-center gap-6 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200/50 dark:border-white/5 shadow-xl rounded-3xl" data-aos="fade-up" data-aos-delay="100">
              <div class="w-14 h-14 rounded-2xl bg-red-500/10 flex items-center justify-center text-red-500 text-3xl font-semibold shadow-inner">🛡️</div>
              <div>
                <div class="text-3xl font-black text-slate-900 dark:text-white">{{ isNl ? 'مؤمن' : 'Insured' }}</div>
                <div class="text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">{{ isNl ? 'تغطية كاملة' : 'Full Coverage' }}</div>
              </div>
            </div>
            <div class="premium-card p-8 flex items-center gap-6 bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200/50 dark:border-white/5 shadow-xl rounded-3xl" data-aos="fade-up" data-aos-delay="200">
              <div class="w-14 h-14 rounded-2xl bg-red-500/10 flex items-center justify-center text-red-500 text-3xl font-semibold shadow-inner">⭐</div>
              <div>
                <div class="text-3xl font-black text-slate-900 dark:text-white">4.9/5</div>
                <div class="text-sm font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">{{ isNl ? 'تقييم ممتاز' : 'Top Rating' }}</div>
              </div>
            </div>
          </div>
          ...
          <div class="space-y-20">
            ...
            >
              ...
              <div v-if="sec.image" class="w-full lg:w-1/2">
                <div class="h-80 md:h-[450px] rounded-[2.5rem] overflow-hidden shadow-2xl border border-slate-100 dark:border-white/5">
              ...
              <div :class="['w-full', sec.image ? 'lg:w-1/2' : 'max-w-3xl mx-auto text-center']">
                ...
                <div class="text-slate-600 dark:text-slate-300 leading-relaxed text-lg whitespace-pre-line">
                  {{ isNl ? sec.content_nl : sec.content_en }}
                </div>
              </div>
            </div>

            <div v-if="dbSections.length === 0" class="text-center py-20 bg-white dark:bg-slate-900 rounded-[2.5rem] border border-slate-100 dark:border-white/5 shadow-xl">
              <p class="text-slate-500 dark:text-slate-400 text-lg">{{ isNl ? 'لا توجد أقسام فرعية مضافة لهذه الخدمة بعد.' : 'No custom sub-sections added for this service yet.' }}</p>
            </div>
          </div>

          <!-- Booking Call to Action Box -->
          <div class="bg-gradient-to-br from-red-600 to-red-700 rounded-[3rem] p-8 md:p-16 text-white shadow-2xl relative overflow-hidden" data-aos="zoom-in">
            ...
            <div class="relative z-10 flex flex-col lg:flex-row gap-8 lg:gap-12 items-center">
              <div class="lg:w-1/2">
                <h2 class="text-3xl sm:text-4xl md:text-5xl font-black mb-6 leading-tight">
                  {{ isNl ? 'احجز خدمتك الآن' : 'Book Your Service Now' }}
                </h2>
                <p class="text-red-50 text-base sm:text-lg mb-8 leading-relaxed">
                  {{ isNl ? 'جاهزون لمساعدتك في الانتقال بكل سهولة وأمان. تواصل معنا للحصول على عرض سعر مجاني ومخصص.' : 'Ready to make your move completely stress-free? Contact us now to receive a customized free quote.' }}
                </p>
              </div>
              <div class="lg:w-1/2 w-full flex flex-col sm:flex-row gap-4 justify-center">
                <NuxtLink :to="localePath('/contact')" class="px-8 py-4 bg-white text-red-600 rounded-2xl font-black shadow-lg hover:bg-red-50 transition-all active:scale-95 text-center flex items-center justify-center gap-2">
                  {{ isNl ? 'احصل على عرض سعر' : 'Get a Quote' }}
                </NuxtLink>
                <a :href="whatsappLink" target="_blank" class="px-8 py-4 bg-green-500 hover:bg-green-600 text-white rounded-2xl font-black shadow-lg transition-all active:scale-95 text-center flex items-center justify-center gap-2">
                  {{ isNl ? 'تواصل عبر الواتساب' : 'Chat via WhatsApp' }}
                </a>
              </div>
            </div>
          </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start mb-20">
              ...
              <div class="lg:col-span-8 space-y-12">
                ...
                <div class="p-8 md:p-10 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/50 dark:border-white/5 shadow-xl" data-aos="fade-up">
                  ...
                  <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div 
                      v-for="(feature, fIdx) in staticFeatures" 
                      :key="fIdx"
              ...
              <div class="lg:col-span-4 lg:sticky lg:top-32 space-y-8">
                ...
                <div class="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/50 dark:border-white/5 shadow-xl text-center space-y-4" data-aos="fade-up">
                  <div class="inline-flex p-3 rounded-2xl bg-red-500/10 text-red-500 text-2xl">🛡️</div>
...
const dbTitle = computed(() => {
  if (!dbService.value) return ''
  return isNl.value ? dbService.value.title_nl : dbService.value.title_en
})

const dbDesc = computed(() => {
  if (!dbService.value) return ''
  return isNl.value ? dbService.value.description_nl : dbService.value.description_en
})
...
const whatsappLink = computed(() => {
  const tName = dbTitle.value || title.value
  return `https://wa.me/31684094271?text=Hi! I'm interested in your ${tName} service.`
})
...
useSeoMeta({
  title: computed(() => dbTitle.value ? `${dbTitle.value} | MoveIt` : `${title.value} | MoveIt Maastricht`),
  description: computed(() => dbDesc.value || description.value)
})
</script>
